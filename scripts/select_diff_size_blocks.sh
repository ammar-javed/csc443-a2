...
do
    ...
    (cd ../ && ./bin/csv2heapfile ./csv/"$NUM_RECORDS.csv" ./scripts/$FILE_TO_USE ${i}) 

	echo -e "===== A D Block Size: ${block_names[$b]} =====\n"
    ../bin/select $FILE_TO_USE 0 A D ${i} 
    echo -e "\n------------------------------------------\n"

    echo -e "===== A Z Block Size: ${block_names[$b]} =====\n"
    ../bin/select $FILE_TO_USE 0 A Z ${i}
    echo -e "\n------------------------------------------\n"

    echo -e "===== A B Block Size: ${block_names[$b]} =====\n"
    ../bin/select $FILE_TO_USE 0 A B ${i}
    echo -e "\n------------------------------------------\n"
